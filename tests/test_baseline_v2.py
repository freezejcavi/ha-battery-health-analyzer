...
def integrity(state: str = "stable") -> CycleIntegrity:
    return CycleIntegrity(
        ...
        battery_reference_percent=65,
        battery_reference_days=7,
        battery_upshift_pp=35 if state == "probable_boundary" else 0,
        battery_signal="persistent_upshift"
        if state == "probable_boundary"
        else "normal",
        voltage_reference_mv=2948,
        ...
        voltage_signal="persistent_upshift"
        if state == "probable_boundary"
        else "normal",
...
class BaselineV2Tests(unittest.TestCase):
    def test_current_probable_boundary_starts_learning_without_old_history(
        self,
    ) -> None:
        result = assess_guarded_baseline_v2(
            ...
            voltage(3237, p10=3119),
            evidence(),
            integrity("probable_boundary"),
            info(),
        )
    ...
    def test_static_voltage_does_not_require_voltage_baseline(self) -> None:
        ...
        result = assess_guarded_baseline_v2(
            {}, {}, voltage(2600), model, integrity(), info("static")
        )
        self.assertEqual(result.eligibility, "not_required")
        self.assertIsNone(result.candidate_mv)

    def test_left_censored_high_battery_can_bootstrap_with_capped_confidence(
        self,
    ) -> None:
        battery_daily = {f"2026-09-{day:02d}": battery(100) for day in range(1, 8)}
        voltage_daily = {
            f"2026-09-{day:02d}": voltage(3040 + day) for day in range(1, 8)
        }
        result = assess_guarded_baseline_v2(
            battery_daily,
            voltage_daily,
    ...
    def test_low_battery_without_observed_boundary_stays_learning(self) -> None:
        ...
        voltage_daily = {
            f"2026-09-{day:02d}": voltage(3000 + day) for day in range(1, 8)
        }
        result = assess_guarded_baseline_v2(
            battery_daily,
            voltage_daily,
    ...
    def test_historical_probable_boundary_excludes_old_cycle_and_boundary_day(
        self,
    ) -> None:
        battery_daily = {}
        voltage_daily = {}
